...
sigma_ep <- function(z, u) {
  0.2 + (0.5 + 0.25 * sin(2 * pi * u)) * (exp(-5 * z) / (1 + exp(-5 * z)))
}

sigma_xi <- function(z, u) {
  0.5 + (0.4 + 0.2 * cos(2 * pi * u)) * exp(-z^2) * sin(z)
}
...
save_xyz <- function(n, k, rho, n_realization) {
  for (r in 1:n_realization) {
    ...
    y <- rep(NA, n)
    for (t in 1:n){
      x[t] <- f(z[t], t / n, k) + sigma_ep(z[t], t / n) * ep[t]
      y[t] <- g(z[t], t / n, k) + sigma_xi(z[t], t / n) * xi[t]
    }
...
set.seed(2026) # seed for reproducibility and so same errors across complexities
...
# load
# df <- load_xyz(n=750, k=1, rho=0.9, r=3)