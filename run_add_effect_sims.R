...
run_sims <- function(params) {
  ...
  effect_size <- params$effect_sizes
  ...
  if (ci_test == "sievedgcm") {
    ...
    res <- run_iid_ci_tests_sims(n_samples = n_sample, n_realization = n_realization,
                                 alpha = alpha, k_complexity = k_complexity,
                                 effect_size = effect_size, fp = dev_path,
                                 plot_residuals = FALSE,
                                 x_name = c("x"), y_name = c("y"),
...
run_sieve_dgcm_sims <- function(n_samples, n_esti, n_realization, n_boot,
                                alpha, k_complexity, effect_size,
  ...
  for (realization in 1:n_realization){
    ...
    df <- load_xyz(n = n_samples, k = k_complexity,
                   beta = effect_size, r = realization)
...
run_oracle_dgcm_sims <- function(n_samples, n_realization, n_boot, alpha,
  ...
  for (realization in 1:n_realization){
    ...
    y_fitted <- (1 + effect_size) * f_z_perf_est
    x_resid <- df[, x_name] - x_fitted
    y_resid <- df[, y_name] - y_fitted
...
effect_sizes <- c(0.9) # 0, 0.3, 0.6, 0.9 # additive effect sizes
set.seed(4)
### seeds for reproducibility
# 1 for effect size 0
# 2 for effect size 0.3
# 3 for effect size 0.6
# 4 for effect size 0.9
