...
run_sims <- function(params) {
  ...
  c_vec <- seq(2, 10, 2)
  # d_vec <- seq(2, 10, 2)
  d_vec <- c(1) # 1 basis function for covariates for indep test

  # for sieve estimator grid 
  n_esti <- 250

  # Get the current datetime
  start_time <- Sys.time()
  if (indep_test == "sievedgcm") {
  ...
  } else if (indep_test == "corrIndeptest") {
    res <- run_iid_indep_tests_sims(n_samples = n_sample,
                                 n_realization = n_realization,
                                 alpha = alpha, 
                                 k_complexity = k_complexity,
                                 ...
                                 plot_residuals = FALSE,
                                 x_name = c("x"), y_name = c("y"),
                                 z_name = c("z"), indep_test = indep_test)
  } else if (indep_test == "KSIndeptest") {
    res <- run_iid_indep_tests_sims(n_samples = n_sample,
                                 n_realization = n_realization,
                                 alpha = alpha, 
                                 k_complexity = k_complexity,
    ...
    print("Only sievedgcm, oracledgcm, corrIndeptest, and KSIndeptest are supported")
    break
  }
  ...
  file_name <- paste0("indep_test_", indep_test,
                      "_dgp_", "corrshock",
                      "_n_sample_", n_sample,
                      "_alpha_", alpha_name,
                      "_k_", k_str,
...
run_sieve_dgcm_sims <- function(n_samples, n_esti, n_realization, n_boot,
  ...
  for (realization in 1:n_realization){
    ...
    y_fitted <- esti_ts(regressors = data.frame(df[, z_name]), # nolint
                        ...
                        b_time = "Legen", b_timese = "Legen",
                        mp_type = "algeb", s = 1, n_esti = n_esti)
    x_on_z_resid <- df[, x_name] - x_fitted
    y_on_z_resid <- df[, y_name] - y_fitted

    res <- dgcm_test(x_on_z_resid = x_on_z_resid, # nolint
                     y_on_z_resid = y_on_z_resid,
...
run_iid_indep_tests_sims <- function(n_samples, n_realization, alpha,
                                  k_complexity, correlation, fp,
                                  plot_residuals,
                                  x_name, y_name, z_name, indep_test) {
  ...
  for (realization in 1:n_realization){
    ...
    if (indep_test == "corrIndeptest") {
      res <- cortest(df[, x_name], df[, y_name], ties.break = "random") #the robust Pearson test
      if (res$p.value < alpha) {
        rej_counter <- rej_counter + 1
      }
      res_list[[paste0("realization", realization)]] <- res
    }
    if (indep_test == "KSIndeptest") {
      res <- indeptest(df[, x_name], df[, y_name], ties.break = "random") #the robust independence test
      if (res$p.value < alpha) {
        rej_counter <- rej_counter + 1
      }
      res_list[[paste0("realization", realization)]] <- res
...
run_oracle_dgcm_sims <- function(n_samples, n_realization, n_boot, alpha,
  ...
  for (realization in 1:n_realization){
    ...
    f_perf_est <- mapply(f, seq(1, n_samples) / n_samples,
                           MoreArgs = list(k = k_complexity))
    g_perf_est <- mapply(g, seq(1, n_samples) / n_samples,
                           MoreArgs = list(k = k_complexity))                           
    x_fitted <- f_perf_est
    y_fitted <- g_perf_est
    x_on_z_resid <- df[, x_name] - x_fitted
...
indep_tests <- c("corrIndeptest", "KSIndeptest", "oracledgcm", "sievedgcm") 
# make sure the directories exist


### cortest() and indeptest() from https://cran.r-project.org/web/packages/robusTest/robusTest.pdf
# https://rdrr.io/cran/robusTest/src/R/indeptest.R
# https://rdrr.io/github/obouaziz/robusTest0/man/indeptest.html
# https://arxiv.org/pdf/2211.08784v2
...
# 9 for correlation 0
# 10 for correlation 0.3
# 11 for correlation 0.6
# 12 for correlation 0.9
